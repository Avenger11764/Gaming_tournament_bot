...
def get_player_data(user_id: int) -> dict:
    """Retrieves player data from Supabase, checking both string and integer telegram_id."""
...
def get_game_state() -> dict:
    ...
    if not db: return {}
    try:
        response = db.table('settings').select('*').eq('key', 'game_data').execute()
        if response.data and len(response.data) > 0:
...
def update_game_state(updates: dict):
    ...
    if not db: return
    try:
        payload = {'key': 'game_data', 'value': updates}
...
async def handle_buy_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    player_data = get_player_data(user_id)
    if not player_data:
        # Create player profile on the fly if user clicks buy
        player_data = {
            'telegram_id': str(user_id),
            'username': query.from_user.username or None,
            'first_name': query.from_user.first_name or None,
            'in_game_name': query.from_user.first_name or f"Player_{user_id}",
            'status': 'Active',
            'coins': 5,
            'cards': [],
            'status_flags': {}
...
async def use_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if card['requires_target']:
        ...
        # Check MSGC registration checkmark rules:
        sender_msgc = bool(player_data.get('msgc_registered', False))
        target_data = get_player_data(target_user.id)
        target_msgc = bool(target_data.get('msgc_registered', False)) if target_data else False

        # Non-MSGC registered players CANNOT target MSGC registered players
        if not sender_msgc and target_msgc:
            await update.message.reply_text("⛔ MSGC Rules: You are not registered in the MSGC Tournament Bot! Non-MSGC players cannot use power cards on MSGC Tournament players.")
            return

        # MSGC registered players can use cards on other MSGC registered players (or non-MSGC players)
        if sender_msgc and not target_msgc:
            await update.message.reply_text("ℹ️ Target player is not registered in MSGC Bot. Using card in Non-MSGC mode.")
...
async def execute_card_effect(update: Update, context: ContextTypes.DEFAULT_TYPE, user, card_id, target_user, card_args):
    ...
    if result.get('action') == 'trigger_ricochet':
        ...
        potential_targets = [
            ...
            if p['user_id'] != result['data']['attacker_id'] and p['user_id'] != result['data']['original_target_id']
        ]
